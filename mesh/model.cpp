...
Group& Model::createGroup() {
    ...
    Group& ref = Groups[Groups.size() - 1]; 
    return ref;
}
...
bool Model::loadModel(std::string path) {
    ...
    for (int i = 0; i < (int)meshes.size() - 1; i++) {
        ...
        tempMesh.Vertices = associatedVertices;
        tempMesh.TexCoords = associatedTexCoords;
        std::vector<std::vector<glm::vec3>> normals(normalIndices.size());
        for (int l = 0; l < normalIndices.size(); l++) {
            for (int m = 0; m < normalIndices[l].size(); m++) {
                normals[l].push_back(associatedNormals[normalIndices[l][m]]);
            }
        }
        auto tempPolygons = tempMesh.createPolygons(vertexIndices, texIndices, normals);
...
void Model::draw(Shader *shader, bool bind) { //Assumes a shader is bound
    GLCall(glBindVertexArray(m_VertexArrayID));
    uint drawPos = 0;
    for (int i = 0; i < (int)Groups.size(); i++) {
...
void Model::loadToBuffer() {
    ...
    const size_t VERTEX_SIZE = (UsingTexCoords) ? TEXTURE_VERTEX_BYTE_SIZE : STANDARD_VERTEX_BYTE_SIZE;
    ...
    GLCall(glVertexAttribPointer(0, 3, GL_FLOAT, false, VERTEX_SIZE, 0));
    GLCall(glVertexAttribPointer(1, 3, GL_FLOAT, false, VERTEX_SIZE, (void*)(3 * sizeof(float))));
    if (UsingTexCoords) {
        GLCall(glEnableVertexAttribArray(2));
        GLCall(glVertexAttribPointer(2, 2, GL_FLOAT, false, VERTEX_SIZE, (void*)(6 * sizeof(float))));
        std::vector<TextureVertex> vertices;
        for (int i = 0; i < Meshes.size(); i++) {
            auto tempvertices = Meshes[i].getTextureVertices();
            vertices.insert(vertices.end(), tempvertices.begin(), tempvertices.end());
        }
        printf("Allocating %luB of GPU memory!\n", vertices.size() * VERTEX_SIZE);
        GLCall(glBufferData(GL_ARRAY_BUFFER, vertices.size() * VERTEX_SIZE, (void*)vertices.data(), GL_STATIC_DRAW));
        
    } else {
        std::vector<StandardVertex> vertices;
        for (int i = 0; i < Meshes.size(); i++) {
            auto tempvertices = Meshes[i].getStandardVertices();
            vertices.insert(vertices.end(), tempvertices.begin(), tempvertices.end());
        }
        printf("Allocating %luB of GPU memory!\n", vertices.size() * VERTEX_SIZE);
        GLCall(glBufferData(GL_ARRAY_BUFFER, vertices.size() * VERTEX_SIZE, (void*)vertices.data(), GL_STATIC_DRAW));
        ...
        if (myFile.is_open()) {
            for (std::getline(myFile, line); myFile.good(); std::getline(myFile, line)) {
                if (line.find("newmtl") != std::string::npos) {
                    groupReference = &createGroup();
                    groupReference->setName(readName("newmtl", line), false);
                    continue;
                } else if (line.find("illum") != std::string::npos) {
                    groupReference->material.illum = (uint)readFloat("illum", line);
                    continue;
                } else if (line.find("bump") != std::string::npos) {
                    groupReference->material.textures.push_back({readName("bump", line), "bumpMap"});

                } else if (line.find("map") != std::string::npos) {

                    if (line.find("map_Ka") != std::string::npos) {
                        groupReference->material.textures.push_back({readName("map_Ka", line), "ambientTexture"});
                    } else if (line.find("map_Kd") != std::string::npos) {
                        groupReference->material.textures.push_back({readName("map_Kd", line), "diffuseTexture"});
                    } else if (line.find("map_Ks") != std::string::npos) {
                        groupReference->material.textures.push_back({readName("map_Ks", line), "specularTexture"});
                    } else if (line.find("map_Ns") != std::string::npos) {
                        groupReference->material.textures.push_back({readName("map_Ns", line), "specularETexture"});
                    } else if (line.find("map_bump") != std::string::npos) {
                        groupReference->material.textures.push_back({readName("map_bump", line), "bumpMap"});
                    }
                } else {
                    auto i = line.begin();
                    ...
                    case 'K':
                        ...
                        switch(*(i + 1)) {
                            case 'a':
                                ...
                                groupReference->material.ambient = {floats[0], floats[1], floats[2]};;
                                break;
                                }
                            ...
                            case 'd':
                                ...
                                groupReference->material.diffuse = {floats[0], floats[1], floats[2]};;
                                break;
                                }
                            ...
                            case 's':
                                ...
                                groupReference->material.specular = {floats[0], floats[1], floats[2]};
                                break;
                                }
                    ...
                    case 'N':
                        ...
                        if (*(i + 1) == 's') {
                            groupReference->material.specE = readFloat("Ns", line);
                        }
                        else if (*(i + 1) == 'i') {
                            groupReference->material.opticalDensity = readFloat("Ni", line);
                        }
                    ...
                    case 'T':
                        if (*(i + 1) == 'r') {
                            groupReference->material.opacity = 1 - readFloat("Tr", line);
                        }
                        break;

                    case 'd':
                        groupReference->material.opacity = readFloat("d", line);
                        break;
