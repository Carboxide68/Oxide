        ...
        if (ImGui::TreeNode(tmp)) {
            ...
            if (ImGui::Button("Load textures")) {
                for (int j = 0; j < models[i]->Groups.size(); j++) {
                    models[i]->Groups[j].generateTextures();
                }
            }
            if (ImGui::Button("Force generate normals")) {
                for (int j = 0; j < models[i]->Meshes.size(); j++) {
                    models[i]->Meshes[j].updatePolygonNormals(true);
                }
            }
            ImGui::Checkbox("Use tex coords", &models[i]->UsingTexCoords);
...
int main(void) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        ImGui::InputFloat3("Light position", myLight.getPositionReference());
