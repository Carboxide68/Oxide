...
namespace Oxide {
    ...
    template<class T>
    class Uniform : private UniformShell {
    public:
        Uniform(const std::string& name, const T val) : m_ValueType(0), m_ConstVal(val), m_Name(name) {m_TypeMe();}
        Uniform(const std::string& name, const T *const val) : m_ValueType(1), m_PtrVal(val), m_Name(name) {m_TypeMe();}
        Uniform(const std::string& name, Ref<T> val) : m_ValueType(2), m_RefVal(val), m_Name(name) {m_TypeMe();}
        Uniform(const std::string& name, std::function<T()> fn) : m_ValueType(3), m_FnVal(fn), m_Name(name) {m_TypeMe();}
    ...
    private:
        ...
        virtual void m_TypeMe() = 0;

        std::string m_Name;

        const int m_ValueType;

        const std::function<T()> m_FnVal;
    ...
    template<> class Uniform<Texture2D>;
    template<> class Uniform<TextureCubeMap>;
